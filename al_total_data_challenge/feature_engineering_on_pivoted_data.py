from al_total_data_challenge.utilities import any_diff_weights, any_diff_weights_no_zero
import numpy as np
import pandas as pd


def make_derivative(df, start, end, ndiv):
    # Calculate local derivative with Taylor series expansion
...
def make_derivative_no_zero(df, start, end, ndiv):
    # Calculate local derivative with Taylor series expansion, do not use x=0 in the calculation.
    # Typically called with ndiv = 0 to interpolate the data point
...
def make_OLS_derivative(df, start, end):
    # Ordinary Least Squares fitted to data.
    # Return slope, intercept, sqrt(r2)
    n = 1 + end - start
    sum_xi = df.copy()
    sum_xi[:] = 0
    sum_yi = sum_xi.copy()
...
def make_sliding(pivoted_df, base_name, window_size):
    ...
    # Calculate std, mean, min, max on pivoted_df on a centered window_size rolling window
    #
    # Typically, the pivoted_df contains a lot of columns. Each column is a 48h forecast, and the index is range(0, 48).
    #
    # Rolling windows are done manually due to the nature of the train/test/split in the challenge :
    #   - NaN due to out of bounds (< 0 or > 48) is filled with first/last known value
    #   - NaN due to train/test split by challenge organizers are kept as NaN and discarded further down the line
    #
    # Example :
    #
    # window_size : 6
    # time        :    0h-11h    12h-23h     24h-26h     27h-35h       36h-47h
    # value_in    :       NaN        NaN     not NaN     not NaN       not NaN
    # value_out   :       NaN        NaN         NaN     not NaN       not NaN
    #
    ...
    for j in range(0, window_size // 2):
        std.iloc[j, :] = (
            pivoted_df.rolling(window_size)
            .std()
    ...
    for j in range(0, window_size // 2):
        std.iloc[-1 - j, :] = (
            pivoted_df.rolling(window_size).std().shift(-j).iloc[-1 - j, :]
        )
...
def make_derivatives(pivoted_df, base_name):
    ...
    # Calculate many derivatives, many different ways.
    # Typically, the pivoted_df contains a lot of columns. Each column is a 48h forecast, and the index is range(0, 48).
...
def make_OLS_derivatives(pivoted_df, base_name):
    ...
    # Calculate derivatives with Ordinary Least Squares
