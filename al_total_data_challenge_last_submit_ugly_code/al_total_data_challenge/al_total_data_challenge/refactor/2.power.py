...
#     'farms'     : list(range(1, 7)),
#     'hh4ofdays' : list(range(0, 6)),
#     'catboost_param':{'n_estimators' : 300}
# },
# {
#     'name'      : 'f',
#     'horizons'  : list(range(0, 4)),
...
from package.ts_split_predict import ts_split_predict, ts_split_predict_quant, ts_split_predict_power
for model in models:
    pred = ts_split_predict_power(data, split_dates_start, split_dates_end, features, model)
...
for farm_number in range(1, 7):
    for s, e in zip(xv_start, xv_end):
        ...
        cb = QuantileCBRegressor(catboost.CatBoostRegressor(n_estimators=1000, bootstrap_type='Bernoulli', subsample=0.8, verbose=1, loss_function='MAE'))
